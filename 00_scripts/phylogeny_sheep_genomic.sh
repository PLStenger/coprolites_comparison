...
# index REF complet si besoin (déjà fait chez toi normalement)
if [[ ! -f "${REF}.bwt" ]]; then
...
# extraction chr1 de la REF si pas déjà fait
if [[ ! -f "$REF_CHR1" ]]; then
...
for NAME in "${!MODERN_GENOMES[@]}"; do
  ...
  # Hypothèse simple: contig nommé '1' ou 'chr1', sinon premier contig
  zcat "$GENOME_PATH" | awk '
    ...
    /^>/ {
      ...
      if (keep==1) { exit } # si on a déjà pris chr1, on sort
      headers[++n]=$0
      next
    }
    {
    ...
    END {
      if (keep==0) {
        # aucun chr1 explicite, on prend le premier contig
        print headers[1]
        print seq[1]
      }
    }
