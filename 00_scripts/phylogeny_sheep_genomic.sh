...
# ========== ÉTAPE 1 : Index de la référence ==========
echo ""
echo "ÉTAPE 1: Indexation de la référence"
echo "----------------------------------------"
...
echo "----------------------------------------"
echo "  Stratégie: pseudogénomes basés sur alignement vs référence chr1"
...
for NAME in "${!MODERN_GENOMES[@]}"; do
    ...
    echo "  $NAME: extraction chromosome 1..."
    
    # On suppose que le premier contig ou un contig nommé '1' correspond au chr1 de référence.
    # Tu peux adapter cette partie si besoin (mapping table).
    zcat "$GENOME_PATH" | \
    awk '
      /^>/{
        h=$0;
        if(n==0){print h}else{exit};
        n++; 
        next
      }
      {print}
...
echo "  Note: on utilise un seul contig/chr1 par génome moderne."

# ========== ÉTAPE 3 : SNP calling échantillons anciens (CHR1, BAM merged) ==========
echo ""
