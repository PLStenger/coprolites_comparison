...
echo ""
echo "=========================================="
echo "ÉTAPE 8: MapDamage - Analyse des dommages (UNMERGED & MERGED)"
echo "=========================================="
echo ""
...
echo " KRAKEN_MERGED: $KRAKEN_MERGED"
if [[ -d "$KRAKEN_MERGED" ]]; then
    N_KRAKEN_M=$(ls -1 "$KRAKEN_MERGED"/*.kraken 2>/dev/null | wc -l)
    echo " ✓ Existe ($N_KRAKEN_M fichiers .kraken)"
else
    echo " ✗ MANQUANT (pas de merged pour Kraken ?)"
fi

echo " FINAL_MERGED_DIR: $FINAL_MERGED_DIR"
if [[ -d "$FINAL_MERGED_DIR" ]]; then
    N_FQ_M=$(ls -1 "$FINAL_MERGED_DIR"/*.fastq.gz 2>/dev/null | wc -l)
    echo " ✓ Existe ($N_FQ_M fichiers fastq.gz)"
else
    echo " ✗ MANQUANT (pas de merged fastq ?)"
fi

# ========== SETUP RÉPERTOIRES ==========
DAMAGE_UNMERGED_BASE="${BASE_DIR}/12_mapdamage/unmerged_reads"
DAMAGE_MERGED_BASE="${BASE_DIR}/12_mapdamage/merged_reads"
mkdir -p "$DAMAGE_UNMERGED_BASE" "$DAMAGE_MERGED_BASE"

LOGFILE="${BASE_DIR}/00_scripts/mapdamage_$(date +%Y%m%d_%H%M%S).txt"
touch "$LOGFILE"

MAPPING_INFO_UNMERGED="${BASE_DIR}/11_summary_tables/mapping_bwa_info_unmerged.tsv"
MAPPING_INFO_MERGED="${BASE_DIR}/11_summary_tables/mapping_bwa_info_merged.tsv"
mkdir -p "${BASE_DIR}/11_summary_tables"

echo -e "Sample\tSpecies\tType\tTotal_Reads\tMapped_Reads\tMapping_Rate(%)" > "$MAPPING_INFO_UNMERGED"
echo -e "Sample\tSpecies\tType\tTotal_Reads\tMapped_Reads\tMapping_Rate(%)" > "$MAPPING_INFO_MERGED"
...
calculate_mapping_rate() {
    ...
    if [[ -f "$bam_file" ]]; then
        ...
        echo -e "${sample_name}\t${species}\t${type}\t${total}\t${mapped}\t${rate}" >> "$outfile"
...
run_mapdamage_with_length_dist() {
    ...
    # --no-stats pour éviter les soucis Rcpp/GCC, tout en gardant les graphes
...
echo ""
echo "=========================================="
echo "PHASE 4: Analyse MERGED READS (Single-End) - MapDamage"
echo "=========================================="
echo ""
...
if [[ -d "$KRAKEN_MERGED" ]] && ls "$KRAKEN_MERGED"/*.kraken >/dev/null 2>&1; then
    for KRAKEN_FILE in "$KRAKEN_MERGED"/*.kraken; do
        ...
        MERGED_FQ="${FINAL_MERGED_DIR}/${SAMPLE}_final_merged.fastq.gz"

        if [[ ! -f "$MERGED_FQ" || ! -s "$MERGED_FQ" ]]; then
            echo ""
            echo "⚠ Fichier merged manquant ou vide pour $SAMPLE"
            continue
        fi

        echo ""
        echo "┌─────────────────────────────────────────"
        echo "│ ${SAMPLE} (merged - $(ls -lh $MERGED_FQ | awk '{print $5}'))"
        echo "└─────────────────────────────────────────"
        ...
        for GROUP in "${!VALID_GENOMES[@]}"; do
            ...
            mkdir -p "$OUTDIR"
            OUT_M="${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged.fastq"

            echo ""
            echo " → ${GROUP} (TaxID: ${TAX_ID}) - merged"
            ...
            if [[ ! -f "$OUT_M" || ! -s "$OUT_M" ]]; then
                echo " ⚠ Aucun read extrait pour ${GROUP} (merged)"
                rm -f "$OUT_M" 2>/dev/null
                continue
            fi

            READ_COUNT=$(grep -c "^@" "$OUT_M" 2>/dev/null || echo 0)
            echo " ✓ ${READ_COUNT} reads merged extraits"

            echo " Mapping BWA single-end (merged)..."

            bwa aln -n 0.08 -l 24 -k 2 -q 20 -t 4 "$REF" "$OUT_M" \
                > "${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged.sai" 2>>"$LOGFILE"

            bwa samse "$REF" \
                "${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged.sai" \
                "$OUT_M" 2>>"$LOGFILE" | \
                samtools view -bS - 2>>"$LOGFILE" | \
                samtools sort -o "${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged.sorted.bam" - 2>>"$LOGFILE"

            if [[ ! -f "${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged.sorted.bam" ]]; then
                echo " ✗ Erreur lors du mapping merged"
                continue
            fi

            samtools index "${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged.sorted.bam" 2>>"$LOGFILE"
            echo " ✓ Mapping merged terminé"

            calculate_mapping_rate "${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged.sorted.bam" "$SAMPLE" "$GROUP" "merged_SE" "$MAPPING_INFO_MERGED"

            run_mapdamage_with_length_dist \
                "${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged.sorted.bam" \
                "$REF" \
                "${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged_mapDamage" \
                "single"

            rm -f "${OUTDIR}/${KRAKEN_BASE}_${GROUP}_merged.sai" "$OUT_M"
        done
    done
else
    echo "⚠ Aucun fichier .kraken dans ${KRAKEN_MERGED} (pas d'analyse merged)"
fi
...
echo "📊 Statistiques de mapping unmerged: $MAPPING_INFO_UNMERGED"
echo "📊 Statistiques de mapping merged:   $MAPPING_INFO_MERGED"
echo "📝 Log complet: $LOGFILE"
echo "📁 Fichiers BAM + MapDamage:"
echo " - Unmerged: $DAMAGE_UNMERGED_BASE/*/"
echo " - Merged:   $DAMAGE_MERGED_BASE/*/"
echo ""

if [[ -f "$MAPPING_INFO_UNMERGED" ]]; then
    echo "Aperçu des résultats unmerged:"
    echo ""
    column -t "$MAPPING_INFO_UNMERGED"
    echo ""
fi

if [[ -f "$MAPPING_INFO_MERGED" ]]; then
    echo "Aperçu des résultats merged:"
    echo ""
    column -t "$MAPPING_INFO_MERGED"
    echo ""
fi
...
echo "Pipeline MapDamage terminé le $(date +%Y-%m-%d\ %H:%M:%S). Résultats: $DAMAGE_UNMERGED_BASE (unmerged) et $DAMAGE_MERGED_BASE (merged)" | \
